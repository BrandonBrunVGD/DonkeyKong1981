...
Barrel::Barrel() {
    ...
    mAnimating = false;
    mScore = 0;
    mBarrelMoveBounds = Vector2(140.0f, 883.0f);

    // from playscreen
    mBarrelMoveSpeed = 160;
    mBarrelCanGoDown = false;
    mBarrelIsGoingDown = false;
    mBarrelShouldGoDown = false;
    mBarrelFallSpeed = 60;
    mDecline = false;    
...
void Barrel::Update() {
    if (!mBarrelIsGoingDown) {
        mBarrelCanGoDown = CheckCollision((184 * 3.5) + 148, 216 * 3.5, 8 * 3.5, 22 * 3.5) ||
                ...
                CheckCollision((208 * 3.5) + 148, 69 * 3.5, 16 * 3.5, 35 * 3.5) ||
                CheckCollision(100, 102 * 3.5, 16 * 3.5, 35 * 3.5) || 
                CheckCollision((208 * 3.5) + 148, 135 * 3.5, 16 * 3.5, 35 * 3.5) ||
                CheckCollision(100, 168 * 3.5, 16 * 3.5, 35 * 3.5) ||
                CheckCollision((208 * 3.5) + 148, 201 * 3.5, 16 * 3.5, 35 * 3.5);
    }

    mBarrelShouldGoDown = CheckCollision((184 * 3.5) + 148, 216 * 3.5, 8 * 3.5, 22 * 3.5) ||
        CheckCollision((85 * 3.5) + 148, 179 * 3.5, 8 * 3.5, 30 * 3.5) ||
        CheckCollision((32 * 3.5) + 148, 183 * 3.5, 8 * 3.5, 22 * 3.5) ||
        CheckCollision((112 * 3.5) + 148, 146 * 3.5, 8 * 3.5, 30 * 3.5) ||
        CheckCollision((184 * 3.5) + 148, 150 * 3.5, 8 * 3.5, 22 * 3.5) ||
...
void Barrel::BarrelMove() {
    ...
    if (!mBarrelIsGoingDown) {
        if (mBarrelCanGoDown) {
            RandomLadder = (rand() % 15) + 1;
            if (RandomLadder == 1) {
                SetBarrelLadder(true);
                mBarrelIsGoingDown = true;
            }
            else if (RandomLadder > 1) {
                mBarrelIsGoingDown = false;
                mBarrelCanGoDown = false;
                mBarrelShouldGoDown = false;
            }
    ...
    if (mBarrelIsGoingDown && mBarrelShouldGoDown) {   // vertical movement
