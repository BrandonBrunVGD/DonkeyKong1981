...
void PlayScreen::Update() {
    ...
    for (auto b : mBarrels) {
        if (CheckCollision(mPlayer->Position().x + 16, mPlayer->Position().y - 16, mPlayer->Position().x + 32, mPlayer->Position().y - 16, b->Position().x, b->Position().y, 12, 12)) {
    ...
    for (auto b : mBarrels) {
        if (CheckCollision(200, 850, 16 * 3.5, 16 * 3.5, b->Position().x, b->Position().y, 12, 12)) {
            b->Visible(false);
            mIgniteBarrel = 2;
        }
    }
    ...
    if (mPlayer->Position().y <= 168 || mPlayer->GetWasHit() == true) {
        mResetGame = true;
    }
...
bool PlayScreen::GetResetGame() {
    return mResetGame;
}

void PlayScreen::SetResetGame(bool reset) {
    mResetGame = reset;
}
